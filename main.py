...
try:
    open("tmp.py")
    import tmp
    cum_bucks = tmp.cum_bucks
    if tmp.activity is None:
        new_session = True
...
def exit_requested(signum, frame):

    try:
        i = input(
            "You pressed \"ctrl+c\". Type: \n"
            "\t\"1\" to go back\n"
            "\t\"2\" to exit\n"
            "\t\"3\" to exit and save\n"
        )
    except Exception:
        print("You pressed \"ctrl+c\" while waiting for input. Try again.")
        return

    global cum_bucks, activity
...
print(
    "\nIf you don't like to think while making decisions, "
...
if new_session:
    ...
    print("Twisting the sock.")
    loading(times=1)
    print(f"You got: {activity}!")

else:
    try:
        input(
            f"You had {cum_bucks} CumBucks when you left.\n"
            f"Keep doing {activity}, and you'll get more. \n"
    ...
    print(f"\tYou work, you get bucks. Let's go!")
